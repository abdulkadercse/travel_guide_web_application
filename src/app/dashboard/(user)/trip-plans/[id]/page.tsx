...
export default function TripPlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newItem.destinationId) return toast.error("Please select a destination");
    ...
    try {
      ...
      toast.error(errorMessage(err, "Failed to add destination"), { id: toastId });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteItem = async (itemId: string) => {
    if (!confirm("Are you sure you want to remove this spot from your plan?")) return;
    try {
  ...
  if (!plan) {
    return (
      <div className="py-24 text-center space-y-4">
        <h2 className="text-xl font-bold">Trip Plan Not Found</h2>
    ...
    <ProtectedRoute allowedRoles={["USER", "ADMIN", "SUPER_ADMIN"]}>
      <div className="space-y-8 font-sans">
        ...
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" className="rounded-full font-bold text-xs" asChild>
            <Link href="/dashboard/trip-plans">
              <FaArrowLeft className="mr-1.5 h-3.5 w-3.5" /> Back to My Custom Trip Plans
            </Link>
          </Button>
        </div>
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-card border border-border shadow-md space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="px-3 py-1 rounded-full text-xs font-bold bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 inline-flex items-center gap-1.5">
                    <FaCalendarAlt className="h-3 w-3" />
                    {new Date(plan.startDate).toLocaleDateString()} &ndash; {new Date(plan.endDate).toLocaleDateString()}
                  </span>
                </div>
              ...
              <Button
                ...
                variant="outline"
                className="rounded-full font-bold text-xs gap-1.5 shrink-0"
              >
                <FaEdit className="h-3.5 w-3.5 text-indigo-500" /> Edit Plan Details
              </Button>
            </div>
          ) : (
            <form onSubmit={handleSavePlanDetails} className="space-y-4">
              <h2 className="text-lg font-bold">Edit Plan Details</h2>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button type="submit" className="bg-indigo-600 text-white font-bold gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4 border-t border-border">
            <div className="p-4 rounded-2xl bg-card/60 border border-border flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold text-muted-foreground uppercase">Target Budget</p>
                <h3 className="text-xl font-black text-foreground mt-0.5">৳{plan.totalBudget?.toLocaleString() || 0}</h3>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-card/60 border border-border flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold text-muted-foreground uppercase">Estimated Spots Cost</p>
                <h3 className={`text-xl font-black mt-0.5 ${budgetExceeded ? "text-rose-400" : "text-emerald-400"}`}>
                  ৳{estimatedTotalCost.toLocaleString()}
                </h3>
              </div>
              <FaCoins className={`h-6 w-6 ${budgetExceeded ? "text-rose-500" : "text-emerald-500"}`} />
            </div>
            ...
            {budgetExceeded && (
              <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center gap-3 col-span-full lg:col-span-1">
                ...
                <p className="text-xs text-rose-400 font-semibold leading-tight">
                  Budget Warning: Estimated spots cost exceeds your target budget by ৳{(estimatedTotalCost - (plan.totalBudget || 0)).toLocaleString()}!
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <div>
              <h2 className="text-xl font-bold tracking-tight">Itinerary Destination Spots</h2>
              <p className="text-xs text-muted-foreground">Destinations added to this trip plan</p>
            </div>

            <Dialog open={itemDialogOpen} onOpenChange={setItemDialogOpen}>
              <DialogTrigger>
                <Button className="bg-indigo-600 hover:bg-indigo-500 text-white rounded-full font-bold shadow-md shadow-indigo-600/20 gap-2">
                  <FaPlus className="h-3.5 w-3.5" /> Add Spot to Itinerary
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-md bg-card border-border">
                <DialogHeader>
                  <DialogTitle className="text-lg font-bold">Add Destination to Plan</DialogTitle>
                </DialogHeader>

                <form onSubmit={handleAddItem} className="space-y-4 pt-2">
                  <div className="space-y-1">
                    <label className="text-xs font-semibold">Select Destination</label>
                    <select
                  ...
                  <div className="space-y-1">
                    <DatePicker
                      ...
                      onChange={(val) => setNewItem({ ...newItem, visitDate: val })}
                  ...
                  <div className="pt-2 flex justify-end gap-2">
                    ...
                    <Button type="submit" disabled={submitting} className="bg-indigo-600 text-white font-bold">
                      {submitting ? <FaSpinner className="animate-spin mr-1" /> : "Add to Plan"}
                    </Button>
                  </div>
                </form>
              </DialogContent>
          ...
          {(!plan.items || plan.items.length === 0) ? (
            <div className="py-12 text-center text-muted-foreground space-y-3 rounded-3xl border border-dashed border-border bg-card/50 p-8">
              <FaMapMarkerAlt className="h-10 w-10 text-muted-foreground/40 mx-auto" />
              <h3 className="text-base font-bold text-foreground">No spots added to itinerary yet</h3>
              <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                Click "Add Spot to Itinerary" above to include tourist destinations in your schedule.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {plan.items.map((item: any) => (
                <div
                  key={item.id}
                  className="p-5 rounded-2xl bg-card border border-border shadow-sm flex items-start justify-between gap-4 hover:border-indigo-500/40 transition-all"
                >
                  <div className="flex items-start gap-4">
                    {item.destination?.coverImage && (
                      <div className="relative h-16 w-20 rounded-xl overflow-hidden bg-slate-900 border shrink-0">
                        <Image
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        {item.destination?.location || item.destination?.district}
                      </p>
                      {item.visitDate && (
